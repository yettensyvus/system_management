...
namespace dashboard
{
    public partial class frm_peoples_death : Form
    {
        ...
        private void all_data()
        {
            ...
            {
                conn.ConnectionOpen();
                SqlDataAdapter sda = new SqlDataAdapter("SELECT id_deces AS ID, full_name AS NUME, date_of_birth AS DATE, idnp AS IDNP, date_of_death AS DECES FROM peoples_deces  " + query, conn.connection);
